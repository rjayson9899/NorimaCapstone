	...
	public static void main(String[] args) {
		...
		do {
			...
			switch (choice) {
				 ...
				 * is found, the policy details are printed for review by the user. The program
				 * will then check if the policy is still eligible for cancellation by checking
				 * if it is expired. If it is expired, the program will return back to the main
				 * menu. Otherwise, the program will ask the user if they want to cancel the
				 * policy. If they input "y", regardless of case, the policy will be canceled.
				 * Any other input will abort the cancellation process.
				 * 
				 * Inputs:
				 * 		(int)		inputID - The ID of the Policy to cancel
				 * 		(String)	strIn	- confirmation if policy is to be cancelled or not
				 * 
				 * Case makes use of the following helper methods:
				 * 		> getPositiveIntLimitedInput()
				 */
				case 3:
					...
					for (CustomerAccount custObj: customerList) {
						...
						if (custObj.hasPolicy(inputId)) {
							// Prints policy details and asks user if they wish to cancel this policy
							System.out.println("\nPolicy found, review details below:");
							...
							if (custObj.getPolicyMatchingId(inputId).isExpired()) {
								...
								// Asks user if they want to cancel the policy
								System.out.print("\nCancel this policy? [y for yes, anything else for no]: ");
								strIn = in.nextLine();
								if (strIn.equalsIgnoreCase("y")) {
									...
									System.out.println("\nPolicy cancellation aborted");
								}
							}
							
							// Raise flag that hit was found
							foundHit = true;
						}
					}
				 ...
				 * claim ID. Unique ID is generated by calling Claim.generateUniqueId() method. the program
				 * will only proceed only if a unique ID is generated which will then proceed to call makeClaim() 
				 * to create a claim instance.
				 * 
